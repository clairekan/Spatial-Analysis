...
#####設定α=0.05。從圖中可以發現隨機預估的平均最短距離分布範圍約落在1200~1450m之間，而學校點與點之間的平均最短距離(藍線)位在約960m的位置。證明隨機預估的平均最短距離分布與學校點與點之間的平均最短距離(藍線)有很顯著的差異。
...
#####設定α=0.05。上圖兩條紅線分別是隨機分布平均前100短的距離的第5分位與第95分位，可以發現學校的前100短距離的縣並不在兩條紅線之間，代表差異非常顯著。
<br/>
...
#####設定α=0.05。上圖紅線代表學校的機率累積分布圖，可以看出一開始短距離的部分上升的很快，並且跟隨機分布的黑線差異很顯著。
<br/>
